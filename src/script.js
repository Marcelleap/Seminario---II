  ...
  try {
    ...
    while (current <= nextEndDate) {
      ...
      if (dayOfWeek >= 0 && dayOfWeek <= 6) {
        const formattedDate = `${current.getFullYear()}-${current.getMonth() +
          1}-${current.getDate()}`
...
function createEditModal (callback) {
  ...
  saveButton.addEventListener('click', function () {
    const editedContent = document.getElementById('editModalInput').value
    const currentCell = modal.currentCell
    if (currentCell) {
      currentCell.textContent = editedContent
      callback(currentCell, editedContent)
    }
    closeModal()
  })
}

function openModal (cellContent, callback) {
  const editModal = document.getElementById('editModal')
  if (!editModal) {
    createEditModal(callback)
  }

  document.getElementById('editModalInput').value = ''

  editModal.currentCell = cellContent
...
function createTableWithClasses (classesRange) {
  try {
    ...
    classesRange.forEach((classInfo, index) => {
      ...
      editButton.addEventListener('click', function () {
        openModal(cellContent, function (editedContent) {
          cellContent.innerHTML = editedContent
        })
      })
